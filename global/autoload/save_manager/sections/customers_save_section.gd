...
# Save section: nightly customer state — the active customer set and the day's
# resolved sales ledger. Reads/writes the MetaManager singleton; registered with
# SaveManager as the "customers" section.
...
func to_dict() -> Dictionary:
    ...
    for c: Customer in MetaManager.nightly_customers:
        serialized_customers.append(c.to_dict())
    return {
        "nightly_customers": serialized_customers,
        "customer_sales_today": MetaManager.customer_sales_today,
    }


func from_dict(data: Dictionary) -> void:
    MetaManager.nightly_customers = []
    if data.has("nightly_customers") and data["nightly_customers"] is Array:
        for d: Variant in data["nightly_customers"]:
            if d is Dictionary:
                MetaManager.nightly_customers.append(Customer.from_dict(d))

    MetaManager.customer_sales_today = []
    if data.has("customer_sales_today") and data["customer_sales_today"] is Array:
        for rec: Variant in data["customer_sales_today"]:
            if rec is Dictionary:
                ...
                MetaManager.customer_sales_today.append(rec)

...
- [ ] `warehouse_entry_scene.tscn` with `warehouse_closed.png` → `warehouse_open.png` animation
- [ ] Run init logic: generate `GameManager.item_entries` (MVP: `is_veiled = false` for all, `resolved_veiled_type = null`)
- [ ] `GameManager.go_to_warehouse_entry()` method and scene path constant
- [ ] Hub "Next Auction Run" routes to this scene instead of directly to Block 02

## Itch Demo Todolist

- [ ] Respect `ItemData.veiled_types`: roll `is_veiled` and pick `resolved_veiled_type` per item at entry
- [ ] Lot number display on warehouse exterior (e.g. `#07` sign — already present in asset)

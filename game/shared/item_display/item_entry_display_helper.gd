...
static func rarity_text(entry: ItemEntry) -> String:
	if entry.is_veiled() or entry.item_data == null:
		return UNKNOWN_TEXT

	var r: int = entry.item_data.rarity
...
static func display_name_color(entry: ItemEntry) -> Color:
	if entry.is_veiled() or not entry.verified or entry.item_data == null:
		return Color.WHITE
	match entry.item_data.rarity:
		...
		_:
			push_warning("Unknown rarity: %d" % entry.item_data.rarity)
	return Color(0.85, 0.85, 0.85)
...
static func sort_value(entry: ItemEntry, column: int) -> Variant:
	match column:
		...
		ItemRow.Column.RARITY:
			...
			return verified_bonus + float(entry.item_data.rarity)

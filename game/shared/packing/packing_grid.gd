...
## Initialises the grid with the given dimensions. Callbacks should be set
## before calling this so _ready cell construction can use them.
...
func refresh_visuals() -> void:
	...
	if phase == Phase.ITEM_HELD and hover_cell != Vector2i(-1, -1) and active_item != null:
		preview_valid = can_place(active_item, hover_cell)
		for c: Vector2i in get_active_cells(active_item):
			preview_cells.append(hover_cell + c)
		...
		if pos in preview_cells:
			...
			if phase == Phase.ITEM_HELD and active_item == entry:
				...
				style = _make_stylebox(
					...
					_resolve_border_color(entry).lightened(0.15),
				)
			else:
				style = _make_stylebox(
					_resolve_color(entry),
					_resolve_border_color(entry),
				)
		else:

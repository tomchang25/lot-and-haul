...
- **Error guards**: never use `assert()`. It is stripped in release exports. Use `if` + `push_error` + `ToastManager` guards instead. Read `dev/standards/error_guard_standard.md` for the three categories (runtime guard, programmer error, recovery warning) and the pattern for each.
...
- **Scene architecture** (creating or editing block scenes/components): read `dev/standards/block_scene_architecture_standard.md` — covers signal connections, instantiating packed scenes, and the `setup()`/`_apply()` pattern. No-`[connection]`-in-`.tscn` is **lint-enforced** — see `dev/standards/standards_enforcement.md`.

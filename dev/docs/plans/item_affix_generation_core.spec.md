...
- `ItemEntry.rarity` is a computed property equal to the hidden-clue count (`item_entry.gd:43-45`, `Economy.rarity_for_clue_count(hidden_clues.size())`). This spec changes the *source* of `hidden_clues` (now from combinations) but does not touch the rarity mechanism. A no-affix item has zero hidden clues and is therefore lowest rarity — an intended consequence, not a regression.
...
| --- | --- | --- |
| `data/definitions/affix_data.gd` | Medium (new) | `AffixData` resource: `affix_id`, `naming_slot`, `display_name`, `category_scope`, `rarity_weight`, `combinations: Array[AffixCombinationData]`. |

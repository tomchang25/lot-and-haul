...
- Scene rule: **scenes never mutate an Entry directly**. Every mutation goes through a thin wrapper on the owning System. The wrapper calls the entry's mutator and emits any resulting EventBus signal.

## Store (`store/`)

A System-held domain state container with invariant-guarding mutators. Owns the live fields for one domain and their save payload.
...
- Scene rule: scenes read Store fields directly through the System but never call Store mutators — they go through System methods that coordinate the mutation, signalling, and save.
...
Scenes, UI blocks, and display helpers never mutate an Entry or Store directly. All mutations flow through the owning System:

```
Scene → System.wrapper(entry, ...) → entry.mutator() → EventBus.signal
```

- The System wrapper is the sole caller of the Entry's mutators.
- The wrapper emits the appropriate EventBus signal on success.
- Cross-system communication: direct call when transactional (caller correctness depends on outcome); EventBus signal when notification (caller correctness is independent).
- System-level writes (lot generation, storage registration, legacy migrations) are legitimate direct writes by the owning systems — the mediation rule governs scenes and UI code, not the Systems and pipelines that own the data.

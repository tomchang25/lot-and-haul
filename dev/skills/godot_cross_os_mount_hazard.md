...
Do not run Godot editor/import/headless against a cross-OS bind mount. For this project, Godot may only run against an OS-native project directory or the `/tmp` snapshot from `dev/agent_rules/godot_test_check.md`, and that `/tmp` must itself be container-native Linux storage.

...
Never run either layer against the mounted working tree (`sandbox_environment.md`). Build a `/tmp` snapshot as in `godot_headless_check.md`, and ensure `/tmp` is container-native Linux storage, not a Windows bind mount such as `E:/tmp:/tmp`: mktemp dir, `git checkout-index`, copy `dev/tools/bin`, regenerate `data/tres/`, render SFX, `rm -rf .godot`, `--import`. The `--import` step is required for Layer 2 (smoke test loads scenes that depend on imported WAV resources); Layer 1 (unit tests) may work without it but still benefits from a clean import. Then:

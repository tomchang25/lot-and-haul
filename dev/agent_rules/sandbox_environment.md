...
- Consequence for testing: **never run the Godot headless binary against the mounted working tree** — it reads the truncated views and reports bogus parse errors (e.g. `Could not find type "i"`). See `godot_test_check.md` for the safe procedure.

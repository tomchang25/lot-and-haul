...
def check_categories(
    ...
    db_cats = {
        r[0]: {"super_category": r[1], "display_name": r[2], "weight": r[3], "grid_size": r[4]}
        for r in cur.execute(
    ...
    for f in sorted(categories_dir.glob("*.tres")):
        ...
        if category_id not in db_cats:
            rows.append(Row("categories", category_id, "only_tres",
                            ["exists on disk, missing from DB"]))
            continue
        ...
        rows.append(Row("categories", category_id,
                        "mismatch" if diffs else "ok", diffs))

    for cat_id in db_cats:
        if cat_id not in tres_cats:
            rows.append(Row("categories", cat_id, "only_db",
                            ["exists in DB, no .tres file on disk"]))
...
def check_identity_layers(conn: sqlite3.Connection, layers_dir: Path) -> list[Row]:
    ...
    for f in sorted(layers_dir.glob("*.tres")):
        ...
        if layer_id not in db_layers:
            rows.append(Row("identity_layers", layer_id, "only_tres",
                            ["exists on disk, missing from DB"]))
            continue
    ...
    for lid in db_layers:
        if lid not in tres_layers:
            rows.append(Row("identity_layers", lid, "only_db",
                            ["exists in DB, no .tres file on disk"]))
...
def check_items(
    ...
    for f in sorted(items_dir.glob("*.tres")):
        ...
        if item_id not in db_items:
            rows.append(Row("items", item_id, "only_tres",
                            ["exists on disk, missing from DB"]))
            continue
        ...
        rows.append(Row("items", item_id,
                        "mismatch" if diffs else "ok", diffs))

    for iid in db_items:
        if iid not in tres_items:
            rows.append(Row("items", iid, "only_db",
                            ["exists in DB, no .tres file on disk"]))

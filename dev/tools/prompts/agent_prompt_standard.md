...
Two to four sentences. The _goal_, not the _how_. Answer: what feature, which
...
- What is explicitly _out of scope_ (e.g. "rotation is not persisted",
  "extra slot does not affect weight HUD")

### 4. Key data relationships / API

This is where code _is_ warranted. Include:
...
- **Step-by-step by file** — when the change spans multiple files: group changes
  under each file, describe the edit in prose, show code _only_ for non-obvious
...
Short list of things the agent must _not_ touch, must _not_ break, or must leave
...
5. What must it _not_ touch? (constraints)

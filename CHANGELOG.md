...
`systems/` describes the system as it *is* (present tense, no Done lists), `ROADMAP.md`
holds only the *active* flow and decided-but-deferred work, and `TODO.md` holds only
open items. When a flow ships, append one entry here, then cut it from `ROADMAP.md` and
delete its `TODO.md` lines.
